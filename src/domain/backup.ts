...
export function parseBackup(input:string|unknown):{state:State;version:1|2;migrated:boolean;summary:{shifts:number;receipts:number};warnings:string[]} {
 ...
 if(document.format!==BACKUP_FORMAT||(document.version!==1&&document.version!==2))throw new Error('Versión de copia no compatible; no se ha cambiado ningún dato.');
 const state=cleanState(document.version===1?migrateLegacyState(document.state):stateSchema.parse(document.state));
 return {state,version:document.version,migrated:document.version===1,summary:{shifts:state.shifts.length,receipts:Object.values(state.months).filter(m=>m.actual).length},warnings:document.version===1?['Se han conservado los centros originales. Revisa municipio y fechas efectivas de cambio de año.','La copia anterior no incluía fin de residencia: se proponen cinco años. Corrige la fecha en Ajustes.']:[]};
}
