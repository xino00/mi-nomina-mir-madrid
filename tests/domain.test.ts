...
describe('Copias versionadas',()=>{
 it('exporta e importa v4 sin cambios',()=>{
  const s=profile();s.shifts=[shift(s)];s.months['2026-09']=saveReceipt(blankMonth(),actual);
  const result=parseBackup(exportBackup(s));expect(result.version).toBe(4);expect(result.state).toEqual(s);expect(result.summary).toEqual({shifts:1,receipts:1});
 });
 ...
 it('rechaza versiones futuras, estructura inválida y JSON incompleto',()=>{
  expect(()=>parseBackup({format:'mi-nomina-guardias',version:5,state:profile()})).toThrow(/Versión/);
