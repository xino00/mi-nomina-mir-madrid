...
export const builtinWorkProfiles:WorkProfile[]=[workProfileSchema.parse({
 ...
 centres:structuredClone(legacyCentres.slice(0,3)).map(c=>({...c,scheduleMode:'general',...(c.id==='Hospital'?{
...
function safeReplacement(a:Centre,old:Settings,b:Centre,next:Settings){
 return sameIdentity(a,b)&&JSON.stringify(hours(a,old))===JSON.stringify(hours(b,next))&&JSON.stringify(holidays(a))===JSON.stringify(holidays(b));
}
function definition(c:Centre,s:Settings){
 return JSON.stringify({name:normalized(c.name),municipality:normalized(c.municipality),mode:c.scheduleMode??'general',hours:hours(c,s),aliases:[...new Set(c.aliases.map(normalized))].sort(),holidays:holidays(c)});
}
...
export function applyWorkProfile(state:State,input:WorkProfile):State {
 ...
 const settings:Settings={...current.settings,activeWorkProfileId:profile.id,defaultLabourHours:profile.defaultLabourHours,defaultFestiveHours:profile.defaultFestiveHours,payDelay:profile.payDelay};
...
export function profileFromSettings(settings:Settings,name:string,id:string):WorkProfile {
 return workProfileSchema.parse(structuredClone({id,name,residencyYears:settings.gradeDates.length===5?5:4,centres:settings.centres.filter(c=>!c.archived),defaultLabourHours:settings.defaultLabourHours,defaultFestiveHours:settings.defaultFestiveHours,payDelay:settings.payDelay}));
}
