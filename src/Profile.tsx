...
export function Profile({initial,onSave,busy,onImport,onCancel}:{initial:Settings;onSave:(s:Settings)=>Promise<void>;busy:boolean;onImport?:()=>void;onCancel?:()=>void}){
 ...
 <SafeForm onSubmit={async()=>{setError('');try{if(!start||!end)throw new Error('Indica las fechas de inicio y fin.');const next=settingsSchema.parse({...s,residencyStart:start,residencyEnd:end,gradeDates:s.gradeDates.length?s.gradeDates:buildGradeDates(start,years),profileComplete:true,centres:fresh?[centre]:s.centres});await onSave(next);}catch(e){setError(errorMessage(e));}}}>
